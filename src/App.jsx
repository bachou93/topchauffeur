...
const T = {
  fr: {
    ...
    fromLabel: "📍 Adresse de départ", fromPlaceholder: "Ex: 12 Rue de Rivoli, Paris 75001",
    toLabel: "🏁 Adresse d'arrivée", toPlaceholder: "Ex: Aéroport Orly, Terminal 4",
    ...
    refLabel: "Référence", smsNote: "📱 SMS envoyé au", newBooking: "+ Nouvelle réservation",
    contactTitle: "Contactez-nous", footer: "© 2026 Topchauffeur · 24h/24 · 7j/7",
    orlyLabel: "Forfait Aéroport Orly", cdgLabel: "Forfait Aéroport CDG",
  },
  en: {
    brand: "Topchauffeur", brandSub: "Premium VTC · Paris & Île-de-France",
    ...
    fromLabel: "📍 Pickup address", fromPlaceholder: "Ex: 12 Rue de Rivoli, Paris 75001",
    toLabel: "🏁 Drop-off address", toPlaceholder: "Ex: Orly Airport, Terminal 4",
    ...
    refLabel: "Reference", smsNote: "📱 SMS sent to", newBooking: "+ New booking",
    contactTitle: "Contact us", footer: "© 2026 Topchauffeur · 24/7",
    orlyLabel: "Orly Airport Package", cdgLabel: "CDG Airport Package",
  },
  ar: {
    brand: "توب شوفير", brandSub: "نقل VTC المميز · باريس وضواحيها",
    heroTitle: "احجز سائقك\nفي باريس وضواحيها",
    heroSub: "دقيق · محترف · متاح 24 ساعة",
    step1: "الرحلة", step2: "بياناتك", step3: "الدفع",
    tripTitle: "رحلتك",
    fromLabel: "📍 عنوان الانطلاق", fromPlaceholder: "مثال: 12 Rue de Rivoli, Paris 75001",
    toLabel: "🏁 عنوان الوصول", toPlaceholder: "مثال: مطار أورلي، المحطة 4",
    ...
    refLabel: "المرجع", smsNote: "📱 تم إرسال رسالة SMS إلى", newBooking: "+ حجز جديد",
    contactTitle: "تواصل معنا", footer: "© 2026 توب شوفير · 24 ساعة / 7 أيام",
    orlyLabel: "باقة مطار أورلي", cdgLabel: "باقة مطار CDG",
  },
  es: {
    brand: "Topchauffeur", brandSub: "VTC Premium · París & Île-de-France",
    ...
    tripTitle: "Su viaje",
    fromLabel: "📍 Dirección de origen", fromPlaceholder: "Ej: 12 Rue de Rivoli, Paris 75001",
    toLabel: "🏁 Dirección de destino", toPlaceholder: "Ej: Aeropuerto Orly, Terminal 4",
    ...
    refLabel: "Referencia", smsNote: "📱 SMS enviado al", newBooking: "+ Nueva reserva",
    contactTitle: "Contáctenos", footer: "© 2026 Topchauffeur · 24h/24 · 7d/7",
    orlyLabel: "Forfait Aeropuerto Orly", cdgLabel: "Forfait Aeropuerto CDG",
  },
};
...
function StripePaymentForm({ amount, from, to, name, phone, date, time, onSuccess }) {
  ...
  async function handlePay() {
    ...
    try {
      const res = await fetch("/api/create-payment-intent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount, from, to, name, phone, date, time })
      });
  ...
  if (paid) return (
    <div style={{ background: "#dcfce7", border: "1px solid #86efac", borderRadius: 12, padding: 16, textAlign: "center", marginBottom: 16 }}>
      ...
      <p style={{ color: "#16a34a", fontWeight: 700, fontSize: 16 }}>Paiement autorisé — {amount}€</p>
      <p style={{ color: "#166534", fontSize: 12, marginTop: 4 }}>Le montant sera prélevé après confirmation du chauffeur</p>
    </div>
  );

  return (
    <div style={{ background: "#fdfaf6", border: "1.5px solid #e8d9c0", borderRadius: 12, padding: 20, marginBottom: 16 }}>
      <div style={{ fontSize: 13, color: "#555", marginBottom: 12, fontWeight: 600 }}>💳 Coordonnées bancaires</div>
      <div style={{ border: "1.5px solid #e0d8cc", borderRadius: 10, padding: "12px 14px", background: "#fff", marginBottom: 12 }}>
        <CardElement options={{ style: { base: { fontSize: "15px", color: "#1a1a2e", fontFamily: "Georgia, serif" } } }} />
      </div>
      {error && <p style={{ color: "#ef4444", fontSize: 12, marginBottom: 8 }}>⚠️ {error}</p>}
      <button onClick={handlePay} disabled={loading} style={{ width: "100%", background: loading ? "#999" : "linear-gradient(135deg,#635bff,#4f46e5)", color: "#fff", border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer" }}>
        {loading ? "⏳ Traitement en cours..." : `💳 Autoriser ${amount}€`}
      </button>
      <div style={{ fontSize: 11, color: "#16a34a", textAlign: "center", marginTop: 8, fontWeight: 600 }}>🔒 Paiement 100% sécurisé — Stripe · Le montant ne sera débité qu'après confirmation</div>
    </div>
  );
}
      ...
      <div style={{ padding: "0 16px 24px" }}>
        ...
        {!loading && filtered.map(r => (
          <div key={r.id} style={{ ...as.card, borderLeft: `4px solid ${r.status === "confirmed" ? "#22c55e" : r.status === "cancelled" ? "#ef4444" : "#f59e0b"}` }}>
            ...
            {r.payment_intent_id && <div style={{ fontSize: 11, color: "#635bff", margin: "4px 0" }}>💳 Paiement Stripe en attente</div>}
...
function SRow({ label, value }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#555", marginBottom: 7, gap: 8 }}>
      ...
      <span style={{ textAlign: "right", fontWeight: 500, maxWidth: "65%", wordBreak: "break-word" }}>{value}</span>
    </div>
  );
}
    ...
    <header style={s.header}>
      <div style={s.logoArea}>
        <svg width="52" height="32" viewBox="0 0 52 32" fill="none">
          ...
          <circle cx="14" cy="26" r="4" fill="#1a1a2e" /><circle cx="14" cy="26" r="2" fill="#c9a96e" />
          <circle cx="38" cy="26" r="4" fill="#1a1a2e" /><circle cx="38" cy="26" r="2" fill="#c9a96e" />
      ...
      <div style={{ position: "relative" }}>
        <button onClick={() => setOpen(o => !o)} style={s.langDropBtn}>
          {current.label} <span style={{ marginLeft: 6, fontSize: 10 }}>{open ? "▲" : "▼"}</span>
        </button>
        {open && (
          <div style={s.langDropMenu}>
            {LANGS.filter(l => l.code !== lang).map(l => (
              <button key={l.code} onClick={() => { setLang(l.code); setOpen(false); }} style={s.langDropItem}>{l.label}</button>
            ))}
          </div>
        )}
      </div>
    ...
    <div style={{ ...s.page, direction: dir }}>
      ...
      <div style={s.successCard}>
        ...
        <div style={s.refBox}>
          ...
          <span style={s.refCode}>#TC{Math.floor(Math.random() * 90000 + 10000)}</span>
        </div>
...
const s = {
  ...
  langDropBtn: { display: "flex", alignItems: "center", background: "#c9a96e", border: "none", borderRadius: 20, padding: "7px 14px", fontSize: 13, fontWeight: 700, color: "#1a1a2e", cursor: "pointer" },
  ...
  hero: { background: "linear-gradient(135deg,#1a1a2e 0%,#0f3460 100%)", padding: "38px 24px 28px", textAlign: "center" },
