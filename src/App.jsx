...
function calcPrice(from, to, km, t) {
  ...
  if (km > 0) return { price: Math.round(10 + km * 2), label: `10€ + ${km} km × 2€/km`, fixed: false };
  return null;
}
  ...
  try {
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: "user", content: `Distance routière en km entre "${from}" et "${to}" en Île-de-France. Réponds UNIQUEMENT avec un entier.` }]
      })
    });
    const d = await r.json();
    const km = parseInt(d.content?.[0]?.text?.trim());
    return isNaN(km) || km <= 0 ? 10 : km;
  } catch { return 10; }
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fa = detectAirport(from), ta = detectAirport(to);
    if (fa || ta) { setPricing(calcPrice(from, to, 0, t)); return; }
    const timer = setTimeout(async () => {
      setLoadingPrice(true);
      const km = await getDistance(from, to);
      setPricing(calcPrice(from, to, km, t));
      ...
      <div style={s.card}>
        ...
        {step === 3 && <>
          ...
          <div style={s.btnRow}>
            ...
            <button style={{ ...s.confirmBtn, flex: 2, opacity: canStep3 ? 1 : 0.4 }} disabled={!canStep3} onClick={() => setConfirmed(true)}>
              {t.confirm}
            </button>
          </div>
        </>}
  ...
  return (
    <footer style={s.contactFooter}>
      ...
      <div style={s.contactRow}>
        <a href="tel:+33600000000" style={s.contactLink}>📞 +33 6 XX XX XX XX</a>
        <a href="mailto:contact@topchauffeur.fr" style={s.contactLink}>✉️ contact@topchauffeur.fr</a>
      </div>
